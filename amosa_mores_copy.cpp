...
int arcsize;
double Temperature, TMin = 0.0001, TMax = 100, alpha = 0.85;
int  MaxLen, MinLen;
double **area;
int POP_SIZE;
char file[30];
...
void print_archive1();
void read_file(char *,char *,char *,char *);
...
main(int argc, char *argv[]) // argv[1]= distance using tf-idf argv[2]= distance using scp argv[3]= output file name
{

	int i,m,j,k,count,dn,*flag2,ii;
	int dim,p;
	FILE *c_ptr, *plt;
	char c_file_name[600],string[4][500];
	...
	strcpy(string[2],argv[3]);
	strcpy(string[3],argv[4]);
	doc_count=atoi(argv[6]);
	n=doc_count;
	printf("\n**** doc count= %d ****\n",n);
	...
	read_file(string[0],string[1],string[2],string[3]);
	...
	for(m=0;m<POP_SIZE;m++)
	{
		...
		if(no_view>1)
		{
			...
			{         
				    track_count=i;
				ComputeMembership(&(pool[m]),3);
				//printf("Membership computed for view 2\n\n");
				UpdateCenter(&(pool[m]),3);            
			...
			{         
				    track_count=i;
				ComputeMembership(&(pool[m]),4);
				//printf("Membership computed for view 2\n\n");
				UpdateCenter(&(pool[m]),4);            
		...
		for (generations = 0; generations < MAX_GEN; generations++)
		{
				...
				{       
						if(no_view>1)       
						{
							...
							ComputeMembership(&new_pool,3);
							UpdateCenter(&new_pool,3); 
							ComputeMembership(&new_pool,4);
							UpdateCenter(&new_pool,4);           
				...
				for(k=0;k<total_index;k++)
				{
				    if(no_view>1)
				    {
						...
						else if (FitnessIndex[k] == 3) Obj[k] = ComputeFitnessPBM(&new_pool,3);
						else if (FitnessIndex[k] == 4) Obj[k] = ComputeFitnessPBM(&new_pool,4);
				        else if (FitnessIndex[k] == 5) Obj[k] = ComputeFitnessDAI(&new_pool);
				    }
				    else
				    {
	...
	printf("\narcsize=%d\n",arcsize);
	sprintf(c_file_name,"%s_plot_multiview.out",argv[5]);
	...
	for(ii=0;ii<arcsize;ii++)
	{
		...
		sprintf(c_file_name,"%s_clus%d_multiview",argv[5],ii+1);
...
void process_new_dominates_current(void)
{
	...
	{
		archive1=(struct archive_element *)malloc((arcsize+5)*sizeof(struct archive_element));
		area1=(double **)malloc((arcsize+5)*sizeof(double *));
		for(i=0;i<arcsize;i++)
...
void process_new_current_nondominating(void)
{
	...
	{
		archive1=(struct archive_element *)malloc((arcsize+1)*sizeof(struct archive_element));
		area1=(double **)malloc((arcsize+1)*sizeof(double));
		for(i=0;i<arcsize;i++)
		{
			if(no_view>1)
...
void read_file(char *file1,char *file2,char *file3,char *file4)
{
	printf("Yes read file.");
	printf("\n%s",file1);
	printf("\n%s",file2);
	printf("\n%s",file3);		
	printf("\n%s",file4);
 ...
 /*	printf("\n%s",file1);
   ...
   no_view=4;// 0 for single view and >=2 for multiview
   ...
   if(no_view>1)
   {
		...
		for(i=0;i<doc_count;i++)
			for(j=0;j<doc_count;j++)
				fscanf(fp,"%lf",&distance_matrix[2][i][j]);// distance matrix of scp
		fclose(fp);
		...
		for(i=0;i<doc_count;i++)
			for(j=0;j<doc_count;j++)
				fscanf(fp,"%lf",&distance_matrix[3][i][j]);// distance matrix of scp
		fclose(fp);
   }
   n=doc_count;
   d=1;
	...
	for (Index1 = 0; Index1 < MaxLen; Index1++)
	{
		if(Chrom->z[Index1].x[0]!=DUMMY && Chrom->z[Index1].x[0]>=0.0)
	   	{  	
		  	...
		  	for(i=0;i<(doc_count-1);i++)
		  	{
		  		
		  		if(arr[i]>0.0)
		  		{
		  			sum=0.0;
			  		for(j=i+1;j<doc_count;j++)
			  		{
			  			if(arr[j]>0.0 && i!=j)
			  			{
				  			if(view==0 || view==2)
				  			...
				  			if(view==3)
				  				sum+=distance_matrix[2][i][j];
				  			if(view==4)
				  				sum+=distance_matrix[3][i][j];
				  		}
			  		}
...
void CombineCenter(struct archive_element *Chrom)
{
	int i,j,k,max1=0.0,*temp,pos,Index1=0,dn=0;
	temp = (int*)calloc(n,sizeof(int));

	//new ========================
	FILE *fp;
    fp = fopen("Membership1.txt", "w");
	for(i=0;i<MaxLen;i++){
		for(j=0;j<n;j++){
			fprintf(fp,"%f ",Chrom->Membership[1][i][j]);
		}
		fprintf(fp,"\n");
	}
	fclose(fp);

	fp = fopen("Membership2.txt", "w");
	for(i=0;i<MaxLen;i++){
		for(j=0;j<n;j++){
			fprintf(fp,"%f ",Chrom->Membership[2][i][j]);
		}
		fprintf(fp,"\n");
	}
	fclose(fp);

	fp = fopen("Membership3.txt", "w");
	for(i=0;i<MaxLen;i++){
		for(j=0;j<n;j++){
			fprintf(fp,"%f ",Chrom->Membership[3][i][j]);
		}
		fprintf(fp,"\n");
	}
	fclose(fp);
	...
	fp = fopen("Membership4.txt", "w");
	for(i=0;i<MaxLen;i++){
		for(j=0;j<n;j++){
			fprintf(fp,"%f ",Chrom->Membership[4][i][j]);
		}
		fprintf(fp,"\n");
	}
	fclose(fp);
	...
	for(i=0;i<MaxLen;i++)
	{
		...
		{
			for(k=0;k<n;k++)
				temp[k]= (Chrom->Membership[1][i][k] && Chrom->Membership[2][j][k] && Chrom->Membership[3][j][k] && Chrom->Membership[4][j][k]);
    ...
    for (Index1 = 0; Index1 < n; Index1++)
	{
		for (Index2 = 0; Index2 < MaxLen; Index2++)
		{
		    ...
		    if(Chrom->z[Index2].x[0]!=DUMMY)
		    {
                ...
                if(view==3)
                	Di2[Index2] =  distance_matrix[2][i][j];
                if(view==4)
                	Di2[Index2] =  distance_matrix[3][i][j];
			}
		}	
	...
	for(i=0;i<POP_SIZE;i++)
	{
		...
		{          
			if(no_view>1)
			{
				...
				else if (FitnessIndex[j] == 3) Obj1 = ComputeFitnessPBM(&pool[i],3);
				else if (FitnessIndex[j] == 4) Obj1 = ComputeFitnessPBM(&pool[i],4);
				else if (FitnessIndex[j] == 5) Obj1 = ComputeFitnessDAI(&pool[i]);
			}
			else
			{
	     ...
	     /* 	for(i=0;i<MaxLen;i++)
		    ...
		    {          
				if(no_view>1)
				{
					...
					else if (FitnessIndex[j] == 5) Obj1 = ComputeFitnessDAI(&new_pool);
				}
				else
				{
				 ...
				 { 
					...
					{    /* valid center */
						...
						for(dn=0;dn<d;dn++)
						{
			                ...
			                if(view==3)
			                	product= distance_matrix[2][m][p];
			                if(view==4)
			                	product= distance_matrix[3][m][p];
